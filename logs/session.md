...
### [2026-07-12 21:26–21:28] Action: реорганізація golden round-1 raw кадрів у batch_001/
- Why: наступні golden-раунди потребують batch_00N/ конвенцію для `data/golden/raw/`, щоб не змішувати кадри різних раундів у плоскій директорії
- Result: `data/golden/raw/*.png` (22 файли) перенесено в `data/golden/raw/batch_001/`, git визначив як 100% rename (історія не втрачена); перевірено, що переміщення не потребує релінкінгу — CVAT task `golden-v0-round1` (id=4) зберігає власну копію кадрів на сервері за іменем файлу, а `cvat_pull.py` матчить експортовані лейбли за `Path(name).name`, не повним шляхом; закомічено (dc41804) і запушено в main
- Next: наступні golden-раунди кадрів заливати одразу в `batch_002/`, `batch_003/` тощо

### [2026-07-12 22:21] Action: Golden set round 2 (batch_002, 72 кадри)
- Why: продовжити нарощування golden/val set (Phase 1.1) — ціль ~100-150 кадрів
- Result: створено CVAT task `golden-v0-round2` (id=6, host 192.168.72.191:8081) з `data/golden/raw/batch_002/` (72 кадри, без попередніх боксів); власник вручну розмітив 2 класи (pidar, military_vehicle) в CVAT UI; `cvat_pull.py` витягнув анотації в `data/golden/labels/` — 64/72 кадрів batch_002 мають боксі, 8 підтверджено порожні (нема цілей); сумарний golden set (round1+round2) — 95 кадрів, 203 боксі (112 pidar + 91 military_vehicle)
- Next: продовжити нарощувати golden set; коли достатньо — заморозити фінальний golden/val split і почати перший fine-tune цикл

### [2026-07-12 22:48] Action: auto-labeling trial + golden/train_pool data architecture
- Why: прогнати "автоматичний" підхід (детектор сам обирає кадри й боксі) поруч з ручним golden-лейблінгом, щоб порівняти якість/покриття; власник підняв питання розділення даних на перевірені (golden) і неперевірені (auto) — узгоджено: golden = будь-що переглянуте людиною незалежно від походження боксів (з нуля вручну чи auto-suggested-then-corrected), а не тільки hand-drawn-from-scratch
- Result: додано `cv_toolkit/pipeline/auto_ingest_batch.py` — прогонає детектор по 5 випадкових відео з `data/scraper/approved/`, ранжує кадри кожного відео за confidence (та сама логіка, що в `bot/client.py`), бере топ-8 кадрів/відео, пушить усе одним CVAT task (`auto-v0-round1`, id=7, 40 кадрів) з попередніми YOLO→taxonomy боксами; власник переглянув усі 40 кадрів у CVAT, виправив 1 хибний, `cvat_pull.py` витягнув анотації (11/40 кадрів з боксами, 6 pidar + 5 military_vehicle); 40 перевірених кадрів перенесено в `data/golden/raw/batch_003/` (та сама конвенція, що batch_001/002); додано `data/golden/provenance.json` — тегує джерело кожного батча (`manual` для 001/002, `auto_reviewed` для 003) + CVAT task id/name; заведено `data/train_pool/{raw,labels,manifest.json}` — окремий простір для майбутніх auto-прогонів; `auto_ingest_batch.py` тепер завжди зберігає сирі (unreviewed) детекції з confidence туди перед будь-яким CVAT push, незалежно від подальшого ревʼю; сумарний golden set (3 батчі) — 106 кадрів, 214 боксів
- Next: продовжити нарощувати golden set; вирішити коли заморозити golden/val split; робити нові auto_ingest_batch.py раунди в train_pool і вибірково промотити перевірені підмножини в golden

### [2026-07-21] Action: Module-3 Claude Code homework — production sandbox/permissions config
- Why: Uber homework (module 3, Path A) — build a production-ready Claude Code config for a real project, closing levels 1-3 (settings tiers, permissions, sandbox) with level 4 (devcontainer) as optional.
- Result: `.claude/settings.json` — added deny rules (`sudo *`, `git push --force*`) alongside existing secret-file denies; added `sandbox.network.allowedDomains` (api.anthropic.com, github.com, registry.npmjs.org, pypi.org, files.pythonhosted.org, api.telegram.org) matched to actual project dependencies. Untracked `.claude/settings.local.json` from git (`git rm --cached`) and added it to `.gitignore` (was never gitignored before, despite being the Local override tier). Removed a dangling `enabledMcpjsonServers: ["cvat"]` reference in settings.local.json pointing at a `.mcp.json` that no longer exists on disk (lost, likely during the earlier `git filter-repo` security cleanup) — tracked as a backlog item in new `BACKLOG.md` (project had no dedicated backlog file before, only HOT.md's ad hoc reminders). Verified the still-working non-MCP CVAT integration (`cvat_pull.py` against task id=7, 11 labels pulled) is unaffected. Level 4 (devcontainer) explicitly skipped — solo project, no onboarding need.
- Bug found and fixed along the way: Claude Code `2.1.216` (`latest` channel) had a broken sandbox that crashed on *any* bash command (`bwrap: Can't create file at /home/.mcp.json: Permission denied`, later `apply-seccomp: ... nested userns is capability-restricted`) — blocked all bash execution in the user's real terminal session. Root-caused to the `latest` update channel via elimination (ruled out project/local settings, `~/.claude.json` per-project MCP cache, `sudo`/AppArmor kernel restrictions — a plain `bwrap` sanity test worked fine). Fixed by reinstalling on `stable` (`2.1.206`) and pinning `autoUpdatesChannel: stable` in `~/.claude/settings.json`. Confirmed working: `git status`, `cat .env` (clean OS-level `Permission denied`, sandbox filesystem layer verified live).
- Separately: this exact `bwrap: Can't create file at /home/.mcp.json` error persists, unresolved, in the chat-agent interface used for this conversation (not the user's local `claude` terminal, which is fully fixed on `stable`) — the interface's own Bash tool has no host-shell access to diagnose or patch further; deferred as a distinct, lower-priority, non-blocking issue since the user's actual terminal-based Rico workflow is unaffected.
- Next: Restore CVAT MCP server (see BACKLOG.md). Decide whether to ship `sandbox.enabled: true` (documents intent, doesn't run on this host due to nested-userns limitation) or `false` (actually-working state) in the homework submission. Resume eval pipeline (IoU threshold, freeze_eval_set.py) — unrelated, still pending from before this session.
