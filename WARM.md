...
- **Phase 1.1 (NEXT)**: Hand-label frozen golden/val set (~100–150 frames, easy classes: vehicle/military_vehicle/structure) from scratch in CVAT — this is the one place auto-labeling is forbidden. Freeze into immutable `golden/` dir, never mixed into train pool. First golden set has 23 frames (insufficient); must grow.
