...
- **Golden dataset structure (2026-07-13)**: `data/golden/raw/batch_001/` (22 frames, round 1) reorganized into batch subdirectories. Future rounds: batch_002/, batch_003/, etc. CVAT task stores server-side frame copies keyed by filename; cvat_pull.py uses Path(name).name (filename only) — no code hardcodes full path. Git detected 100% rename, no history loss.
- **Phase 1.1 (NEXT)**: Continue hand-labeling golden/val set (~100–150 frames total, currently 23). Freeze into immutable `golden/` dir, never mixed into train pool. Easy classes only: vehicle/military_vehicle/structure. First golden set (batch_001, 23 frames, 2 classes) is insufficient; must grow across multiple batches before freezing final golden/val split.
